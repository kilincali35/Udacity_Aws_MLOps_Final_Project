...
def test(model, test_loader, criterion):
    logger.info("Testing activity started.")
    test_loss = correct = 0
    ...
    logger.info("Testing activity completed.")


def train(model, train_loader, valid_loader, criterion, optimizer, epochs, device):
    logger.info("Training activity started.")
    ...
    logger.info("Training activity completed.")
...
def create_transform(split, image_size):
    ...
    if split == "train":
        ...
        train_transforms = transforms.Compose([
            ...
            transforms.RandomApply([transforms.ColorJitter(0.5, 0.5, 0.5, 0.5)], p=0.1),
            transforms.RandomApply([transforms.RandomAffine(degrees=0, translate=(0, 0.02), scale=(0.95, 0.99))], p=0.1),
            transforms.RandomApply([transforms.RandomChoice([transforms.RandomRotation((90, 90)),
                    transforms.RandomRotation((-90, -90)),])], p=0.1),
    ...
    # These transformations are less, and we want to see data as raw as possible. Only Normalize is added to match train data
    elif split == "valid":
...
def main(args):
    ...
    model = net(args.num_classes, device)
